...
Write-Host "1. Starting Node.js Transcribe proxy server (port 5000)..." -ForegroundColor Cyan
...
try {
    $viteProcess = Start-Process -FilePath "npm.cmd" -ArgumentList "run dev" -NoNewWindow -PassThru -ErrorAction Stop
}
catch {
    Write-Host "Error: Could not start the Vite frontend." -ForegroundColor Red
...
Write-Host "  Transcribe:     ws://localhost:5000/" -ForegroundColor White
Write-Host "  Scam Detection: http://localhost:5000/api/predict-scam" -ForegroundColor White
